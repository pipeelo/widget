export const STR = {
  brandFallback: 'Chat',
  close: 'Fechar chat',
  inputPlaceholder: 'Mensagem',
  inputLabel: 'Escreva sua mensagem',
  send: 'Enviar mensagem',
  attach: 'Anexar arquivo',
  attachCamera: 'Câmera',
  attachGallery: 'Fotos e vídeos',
  attachDocument: 'Documento',
  attachAudio: 'Áudio',
  previewSend: 'Enviar',
  previewCancel: 'Cancelar',
  dropHint: 'Solte o arquivo para enviar',
  emoji: 'Emoji',
  emojiRecent: 'Recentes',
  emojiSmileys: 'Carinhas e pessoas',
  emojiNature: 'Animais e natureza',
  emojiFood: 'Comida e bebida',
  emojiActivities: 'Atividades',
  emojiTravel: 'Viagens e lugares',
  emojiObjects: 'Objetos',
  emojiSymbols: 'Símbolos',
  cameraOpening: 'Abrindo a câmera…',
  takePhoto: 'Tirar foto',
  cameraBlocked: 'Este site não liberou a câmera para o chat.',
  cameraDenied: 'Permita o acesso à câmera para tirar a foto.',
  cameraUnavailable: 'Nenhuma câmera encontrada.',
  cameraFailed: 'Não foi possível abrir a câmera.',
  recordAudio: 'Gravar áudio',
  sendAudio: 'Enviar áudio',
  cancelRecording: 'Cancelar gravação',
  recording: 'Gravando…',
  recordTooShort: 'Gravação muito curta.',
  micDenied: 'Permita o acesso ao microfone para gravar.',
  micBlocked: 'Este site não liberou o microfone para o chat.',
  micUnavailable: 'Nenhum microfone encontrado.',
  audioFailed: 'Não foi possível gravar o áudio.',
  locating: 'Buscando sua localização…',
  geoBlocked: 'Este site não liberou a localização para o chat.',
  geoDenied: 'Permita o acesso à localização para enviar.',
  geoUnavailable: 'Não foi possível determinar sua localização. Tente escrever o endereço.',
  geoTimeout: 'A localização demorou demais. Tente de novo ou escreva o endereço.',
  geoFailed: 'Não foi possível obter sua localização.',
  playAudio: 'Reproduzir áudio',
  pauseAudio: 'Pausar áudio',
  dismissNotice: 'Fechar aviso',
  sending: 'Enviando…',
  sent: 'Enviado',
  notDelivered: 'Não entregue',
  retry: 'Tentar de novo',
  reconnecting: 'Reconectando…',
  historyError: 'Não foi possível carregar a conversa.',
  tryAgain: 'Tentar novamente',
  startError: 'Não foi possível iniciar o chat.',
  fileTooLarge: (limit: string) => `Arquivo muito grande (máx. ${limit}).`,
  fileUnsupported: 'Tipo de arquivo não suportado.',
  documentLabel: 'Documento',
  openFile: 'Abrir',
  imageAlt: 'Imagem enviada na conversa',
  videoUnsupported: 'Seu navegador não reproduz este vídeo.',
  loadingOlder: 'Carregando mensagens…',
  poweredBy: 'Powered by',
  imageLabel: 'Imagem',
  audioLabel: 'Áudio',
  videoLabel: 'Vídeo',
  newMessage: 'Nova mensagem',
  olderChats: 'Ver atendimentos anteriores',
  closedAt: (when: string) => `Atendimento encerrado · ${when}`,
  closedPlain: 'Atendimento encerrado',
  protocol: (value: string) => `Protocolo ${value}`,
  preChatTitle: 'Antes de começar',
  preChatIntro: 'Conte pra gente quem você é — assim o atendimento já começa do jeito certo.',
  preChatName: 'Nome',
  preChatEmail: 'E-mail',
  preChatPhone: 'Telefone com DDD',
  preChatDocument: 'CPF/CNPJ',
  preChatRequired: 'Preencha este campo.',
  preChatEmailInvalid: 'Informe um e-mail válido.',
  preChatPhoneInvalid: 'Informe o telefone com DDD, ex.: (11) 91234-5678.',
  preChatDocumentInvalid: 'Informe um CPF ou CNPJ válido.',
  preChatSubmit: 'Iniciar conversa',
  typing: 'digitando…',
  viewOptions: 'Ver opções',
  closeSheet: 'Fechar',
  copyCode: 'Copiar código',
  codeCopied: 'Código copiado',
  pixTitle: 'Pagamento PIX',
  locationLabel: 'Localização',
  openMap: 'Ver no mapa',
  contactLabel: 'Contato',
  reactedWith: (emoji: string) => `Reagiu com ${emoji}`,
  unsupported: 'Mensagem não suportada neste canal',
  pushInvite: 'Quer um aviso quando a gente responder?',
  pushEnable: 'Quero sim',
};

export const MEDIA_LABELS: Record<string, string> = {
  image: STR.imageLabel,
  sticker: STR.imageLabel,
  audio: STR.audioLabel,
  voice: STR.audioLabel,
  video: STR.videoLabel,
  document: STR.documentLabel,
};
