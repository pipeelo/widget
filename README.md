...
- Pré-chat obrigatório (formulário antes da primeira conversa quando o `setUser` não cobre os campos exigidos — spec em [`pre-chat.md`](./pre-chat.md)) e verificação de identidade (HMAC/JWT — spec em [`identidade.md`](./identidade.md)) são evoluções previstas nos docs, fora deste escopo.
