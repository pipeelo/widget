...
- **PIX** (mensagens `type: order_details`): card dentro do balão com `product_name`, `value` (centavos, formatado em BRL) e o código copia-e-cola com "Copiar código" — Clipboard API (o loader injeta o iframe com `allow="clipboard-write"`) e fallback `execCommand`. QR code fica fora do escopo: quem quiser gera a partir do código.
